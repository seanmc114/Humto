...
function nearestMidiInScale(midiFloat, root, scaleName) {
  const allowedPitchClasses = getScalePitchClasses(root, scaleName);
  let bestMidi = Math.round(midiFloat);
  let bestDistance = Infinity;

  const searchStart = Math.floor(midiFloat) - 12;
  const searchEnd = Math.ceil(midiFloat) + 12;

  for (let candidate = searchStart; candidate <= searchEnd; candidate++) {
    const pc = ((candidate % 12) + 12) % 12;
    if (!allowedPitchClasses.includes(pc)) continue;
...
function scoreKeyFit(midiFloats, root, scaleName) {
  if (!midiFloats.length) return -Infinity;
...
function buildRiff(frames) {
  ...
  const allMidiFloats = smoothed
    .filter(f => f.voiced && f.midiFloat != null)
    .map(f => f.midiFloat);
